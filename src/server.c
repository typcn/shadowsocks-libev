...
#ifdef __APPLE__
#include </usr/local/include/sys/socket.h>
#endif
...
static remote_t *connect_to_remote(struct addrinfo *res, server_t *server);
...
static size_t parse_header_len(const char atyp, const char *data,
                               size_t offset);
...
static void stat_update_cb(EV_P_ ev_timer *watcher, int revents) {
  ...
  if (ip_addr.host == NULL || ip_addr.port == NULL) {
    ...
    snprintf(claddr.sun_path, sizeof(claddr.sun_path), "/tmp/shadowsocks.%s",
             server_port);
...
int create_and_bind(const char *host, const char *port) {
  ...
  /*
   * On Linux, with net.ipv6.bindv6only = 0 (the default), getaddrinfo(NULL)
   * with
  ...
  if (!host) {
    ...
    while (ipv4v6bindall) {
      ...
      ipv4v6bindall =
          ipv4v6bindall->ai_next; /* Get next address info, if any */
    }
  }

  for (/*rp = result*/; rp != NULL; rp = rp->ai_next) {
    listen_sock = socket(rp->ai_family, rp->ai_socktype, IPPROTO_SCTP);
    if (listen_sock == -1) {
      continue;
    }
    ...
    if (rp->ai_family == AF_INET6) {
      ...
      setsockopt(listen_sock, IPPROTO_IPV6, IPV6_V6ONLY, &ipv6only,
                 sizeof(ipv6only));
    }
...
static remote_t *connect_to_remote(struct addrinfo *res, server_t *server) {
  ...
  if (fast_open) {
#ifdef __APPLE__
    ((struct sockaddr_in *)(res->ai_addr))->sin_len =
    ...
    int s = connectx(sockfd, &endpoints, SAE_ASSOCID_ANY,
                     CONNECT_DATA_IDEMPOTENT, &iov, 1, &len, NULL);
    if (s == 0) {
      s = len;
    }
#else
    ssize_t s =
        sendto(sockfd, server->buf->array + server->buf->idx, server->buf->len,
               MSG_FASTOPEN, res->ai_addr, res->ai_addrlen);
#endif
    if (s == -1) {
      if (errno == EINPROGRESS || errno == EAGAIN || errno == EWOULDBLOCK) {
        // The remote server doesn't support tfo or it's the first connection to
        // the server.
...
static void server_recv_cb(EV_P_ ev_io *w, int revents) {
  ...
  if (server->stage == 5) {
    if (server->auth &&
        !ss_check_hash(remote->buf, server->chunk, server->d_ctx, BUF_SIZE)) {
      LOGE("hash error");
      report_addr(server->fd);
      close_and_free_server(EV_A_ server);
      close_and_free_remote(EV_A_ remote);
     ...
     *    +------+----------+----------+----------------+
     *
     *    If ATYP & ONETIMEAUTH_FLAG(0x10) == 1, Authentication (HMAC-SHA1) is
     * enabled.
     *
    ...
    if ((atyp & ADDRTYPE_MASK) == 1) {
      ...
      if (server->buf->len >= in_addr_len + 3) {
        ...
        dns_ntop(AF_INET, (const void *)(server->buf->array + offset), host,
                 INET_ADDRSTRLEN);
        offset += in_addr_len;
      } else {
      ...
      if (server->buf->len >= in6_addr_len + 3) {
        ...
        dns_ntop(AF_INET6, (const void *)(server->buf->array + offset), host,
                 INET6_ADDRSTRLEN);
        offset += in6_addr_len;
      } else {
        LOGE("invalid header with addr type %d", atyp);
    ...
    if (server->buf->len < offset) {
      ...
      memmove(server->buf->array, server->buf->array + offset,
              server->buf->len);
    }
    ...
    if (server->auth &&
        !ss_check_hash(server->buf, server->chunk, server->d_ctx, BUF_SIZE)) {
      LOGE("hash error");
      report_addr(server->fd);
      close_and_free_server(EV_A_ server);
      return;
...
static void server_timeout_cb(EV_P_ ev_timer *watcher, int revents) {
  server_ctx_t *server_ctx =
      (server_ctx_t *)(((void *)watcher) - sizeof(ev_io));
...
int main(int argc, char **argv) {
  ...
  if (manager_address != NULL) {
    ev_timer_init(&stat_update_watcher, stat_update_cb, UPDATE_INTERVAL,
                  UPDATE_INTERVAL);
