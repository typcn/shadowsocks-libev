      ...
      if (mode != TCP_ONLY && request->cmd == 3) {
        ...
        if (request->atyp == 1) {
          ...
          if (acl || verbose) {
            ...
            dns_ntop(AF_INET, (const void *)(buf->array + 4), host,
                     INET_ADDRSTRLEN);
            sprintf(port, "%d", p);
          }
        ...
        if ((acl && (request->atyp == 1 || request->atyp == 4) &&
             acl_match_ip(host))) {
          if (verbose) {
...
int main(int argc, char **argv) {
  ...
  int option_index = 0;
  static struct option long_options[] = {{"fast-open", no_argument, 0, 0},
  ...
  for (i = 0; i < remote_num; i++) {
    ...
    char *port =
        remote_addr[i].port == NULL ? remote_port : remote_addr[i].port;
